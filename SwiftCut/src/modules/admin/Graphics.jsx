...
export const Graphics = () => {
    ...
    const [ventas, setUtilidadVentas] = useState(0);
    ...
    const utilid = (item, price) => {
        setUtilidadNetaJson((((item[2]?.quantity || 0 ) * price?.priceSale) - ((item[0]?.quantity || 0) * price?.priceBuy)));
        setUtilidadJson((((item[2]?.quantity || 0) * 100) / ((item[0]?.quantity || 0) * ((item[0]?.quantity || 0) > (item[2]?.quantity || 0) ? 1 : -1))).toFixed(2));
        setUtilidadVentas(((item[2]?.quantity || 0) * price?.priceSale));
    }
                    ...
                    <div className="flex-1 h-full flex flex-col justify-evenly items-center">
                        <div className="w-6/12 border rounded-lg p-5  flex flex-wrap">
                            <div className="w-4/55">
                                ...
                                <div className="flex flex-col m-2">
                                    <Label value={((priceJson.priceSale * priceJson.priceBuy) / 100).toFixed(2) + "%"} className="text-xs text-green-400" />
                                    <Label value={"Ganacias por kilo"} className="text-xs text-green-400" />
                                </div>
                            ...
                            <div className="ml-3">
                                ...
                                <div className="ml-5"><Label value={"$" + (utilidadNeta || 0)} className="text-lg  " /></div>
                                <div className="">
                                <div><Label value="Ventas" className="text-xs " /></div>
                                <div className="ml-5"><Label value={"$" + (ventas ||0) } className="text-xs  " /></div>
                        ...
                        <div className="w-4/5 h-3/6 flex flex-col justify-center items-center">
                            <div className="text-center border p-12 rounded-full shadow-lg">
                                ...
                                <div>
                                    <Label value={(utilidadJson || 0)} className="text-center text-6xl font-thin" /><span>%</span>
                                </div>
                              

                            </div>
                        </div>
                        <div className="w-4/5 flex flex-col justify-center items-center border rounded-md p-2" >
                            <div>
                                <Label value="Demanda por categoria" className="text-lg" style={{ color: "var(--blackLigth)" }} />
                            </div>
